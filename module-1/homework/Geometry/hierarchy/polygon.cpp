...
bool Polygon::isSimilarTo(const Shape& another) {
  ...
  for (int i = 0; i < v1.size() - 2; i++) {
    ...
    Point v21 = __get__vector__(v2[i], v2[i + 1]);
    Point v22 = __get__vector__(v2[i + 1], v2[i + 2]);
    if (abs(__get__len__(v12)/__get__len__(v11) - __get__len__(v22)/__get__len__(v21)) > 1e-9 ||
        abs(__get__angle__(v11, v12) - __get__angle__(v21, v22)) > 1e-9){
            return 0;
        }
  }
...
bool Polygon::containsPoint(Point point){
    ...
    double sum = 0;
    for (int i = 0; i < vertices.size() - 1; i++){
        sum += __get__angle__(__get__vector__(point, vertices[i]), __get__vector__(point, vertices[i + 1]));
    }
    vertices.pop_back();
    return abs(sum) >= 1e-9;
}
...
void Polygon::rotate(Point center, double angle){
    for(int i = 0; i < vertices.size(); i++){
        ...
        p = Point(p.x*cos(angle) - p.y*sin(angle), p.x*sin(angle) + p.y*cos(angle));
...
void Polygon::reflex(Line axis){
    for(int i = 0; i < vertices.size(); i++){
        double l = 2*((axis.get_a()*vertices[i].x + axis.get_b()*vertices[i].y + axis.get_c())/sqrt(pow(axis.get_a(),2) + pow(axis.get_b(),2)));
...
void Polygon::scale(Point center, double coefficient){
    for(int i = 0; i < vertices.size(); i++){
        ...
        vertices[i] = __sum__(center, __prod__(tmp, coefficient/__get__len__(tmp)));
    }
}